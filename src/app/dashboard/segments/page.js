"use client";
import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import DashboardLayout from "@/components/DashboardLayout";
import getApi from "@/lib/api";

export default function SegmentsPage() {
  const [segments, setSegments] = useState([]);
  const [loading, setLoading] = useState(true);
  const [mounted, setMounted] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [previewing, setPreviewing] = useState(false);
  const [preview, setPreview] = useState(null);
  const [error, setError] = useState("");
  const [success, setSuccess] = useState("");
  const [form, setForm] = useState({
    name: "",
    filters: { tag: "", emailDomain: "", nameContains: "" },
  });
  const router = useRouter();

  useEffect(() => {
    setMounted(true);
    const token = localStorage.getItem("notifiq_token");
    if (!token) return router.push("/login");
    fetchSegments();
  }, []);

  const fetchSegments = async () => {
    try {
      const res = await getApi().get("/segments");
      setSegments(res.data);
    } catch {
      router.push("/login");
    } finally {
      setLoading(false);
    }
  };

  const handlePreview = async () => {
    setPreviewing(true);
    setPreview(null);

    try {
      const filters = Object.fromEntries(
        Object.entries(form.filters).filter(([_, v]) => v !== ""),
      );
      const res = await getApi().post("/segments/preview", { filters });
      setPreview(res.data);
    } catch (err) {
      setError(err.response?.data?.error || "Preview failed!");
    } finally {
      setPreviewing(false);
    }
  };

  const handleCreate = async () => {
    if (!form.name) return setError("Segment name is required");
    setSubmitting(true);
    setError("");
    setSuccess("");

    try {
      const filters = Object.fromEntries(
        Object.entries(form.filters).filter(([_, v]) => v !== ""),
      );
      await getApi().post("/segments", { name: form.name, filters });
      setSuccess("Segment created!");
      setForm({
        name: "",
        filters: { tag: "", emailDomain: "", nameContains: "" },
      });
      setPreview(null);
      fetchSegments();
    } catch (err) {
      setError(err.response?.data?.error || "Something went wrong");
    } finally {
      setSubmitting(false);
    }
  };

  const handleRefresh = async (id) => {
    try {
      const res = await getApi().post(`/segments/${id}/refresh`);
      setSuccess(`Segment refreshed - ${res.data.contact_count} contacts`);
      fetchSegments();
    } catch (err) {
      setError(err.resposne?.data?.error || "Refresh failed");
    }
  };

  const handleDelete = async (id) => {
    try {
      await getApi().delete(`/segments/${id}`);
      fetchSegments();
    } catch (err) {
      setError(err.response?.data?.error || "Refresh failed");
    }
  };

  if (!mounted) return null;

  return (
    <DashboardLayout>
      <div className="px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Segments</h1>
          <p className="text-gray-500">
            Group your contacts and send targeted campaigns.
          </p>
        </div>

        {/* Create form */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
          <h3 className="text-gray-900 font-semibold text-sm mb-4">
            Create Segment
          </h3>

          <div className="mb-4">
            <label className="text-gray-600 text-sm font-medium mb-1.5 block">
              Segment name
            </label>
            <input
              type="text"
              placeholder="e.g. VIP customers"
              value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              className="w-full border border-gray-200 text-gray-900 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition"
            />
          </div>

          <p className="text-gray-600 text-sm font-medium mb-3">
            Filters{" "}
            <span className="text-gray-400 font-normal">
              (leave blank to include all contacts)
            </span>
          </p>

          <div className="grid grid-cols-3 gap-4 mb-5">
            <div>
              <label className="text-gray-500 text-xs mb-1.5 block">
                Has tag
              </label>
              <input
                type="text"
                placeholder="e.g. vip"
                value={form.filters.tag}
                onChange={(e) =>
                  setForm({
                    ...form,
                    filters: { ...form.filters, tag: e.target.value },
                  })
                }
                className="w-full border border-gray-200 text-gray-900 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition"
              />
            </div>
            <div>
              <label className="text-gray-500 text-xs mb-1.5 block">
                Email domain
              </label>
              <input
                type="text"
                placeholder="e.g. gmail.com"
                value={form.filters.emailDomain}
                onChange={(e) =>
                  setForm({
                    ...form,
                    filters: { ...form.filters, emailDomain: e.target.value },
                  })
                }
                className="w-full border border-gray-200 text-gray-900 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition"
              />
            </div>
            <div>
              <label className="text-gray-500 text-xs mb-1.5 block">
                Name contains
              </label>
              <input
                type="text"
                placeholder="e.g. John"
                value={form.filters.nameContains}
                onChange={(e) =>
                  setForm({
                    ...form,
                    filters: { ...form.filters, nameContains: e.target.value },
                  })
                }
                className="w-full border border-gray-200 text-gray-900 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition"
              />
            </div>
          </div>

          {/* Preview */}
          {preview && (
            <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 mb-4">
              <p className="text-blue-700 font-semibold text-sm mb-1">
                {preview.count} contacts match these filters
              </p>
              {preview.contacts.length > 0 && (
                <p className="text-blue-500 text-xs">
                  Preview: {preview.contacts.map((c) => c.email).join(", ")}
                  {preview.count > 5 ? ` and ${preview.count - 5} more...` : ""}
                </p>
              )}
            </div>
          )}

          {error && <p className="text-red-500 text-sm mb-3">{error}</p>}
          {success && <p className="text-green-600 text-sm mb-3">{success}</p>}

          <div className="flex items-center gap-3">
            <button
              onClick={handleCreate}
              disabled={submitting}
              className="bg-gray-900 hover:bg-gray-700 disabled:opacity-50 text-white font-medium rounded-xl px-6 py-3 text-sm transition shadow-sm"
            >
              {submitting ? "Creating..." : "Create Segment"}
            </button>
            <button
              onClick={handlePreview}
              disabled={previewing}
              className="border border-gray-200 text-gray-600 hover:border-gray-400 font-medium rounded-xl px-6 py-3 text-sm transition disabled:opacity-50"
            >
              {previewing ? "Previewing..." : "👁️ Preview contacts"}
            </button>
          </div>
        </div>

        {/* Segments list */}
        <h2 className="text-lg font-semibold text-gray-900 mb-4">
          Your Segments
        </h2>
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-white rounded-2xl h-20 animate-pulse border border-gray-100"
              />
            ))}
          </div>
        ) : (
          <div className="space-y-3">
            {segments.length === 0 && (
              <p className="text-gray-400 text-sm">
                No segments yet. Create one above.
              </p>
            )}
            {segments.map((seg) => (
              <div
                key={seg.id}
                className="bg-white rounded-2xl shadow-sm border border-gray-100 px-6 py-4 flex items-center justify-between"
              >
                <div>
                  <p className="text-gray-900 font-medium text-sm">
                    {seg.name}
                  </p>
                  <p className="text-gray-400 text-xs mt-0.5">
                    {seg.contact_count} contacts
                  </p>
                  <div className="flex items-center gap-2 mt-1.5">
                    {Object.entries(seg.filters || {})
                      .filter(([_, v]) => v)
                      .map(([k, v]) => (
                        <span
                          key={k}
                          className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full font-mono"
                        >
                          {k}: {v}
                        </span>
                      ))}
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <a
                    href={`/dashboard/blast?segmentId=${seg.id}&segmentName=${encodeURIComponent(seg.name)}`}
                    className="text-xs font-medium px-3 py-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition"
                  >
                    ⚡ Blast
                  </a>
                  <button
                    onClick={() => handleRefresh(seg.id)}
                    className="text-xs font-medium px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:border-gray-400 transition"
                  >
                    Refresh
                  </button>
                  <button
                    onClick={() => handleDelete(seg.id)}
                    className="text-xs font-medium px-3 py-1.5 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition"
                  >
                    Delete
                  </button>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
    </DashboardLayout>
  );
}
